...
#define LOCTEXT_NAMESPACE "DungeonFlowEditorUtils"

DEFINE_LOG_CATEGORY_STATIC(LogDungeonFlowEditorUtils, Log, All);

/////////////////////////////// FDungeonFlowEditorUtils /////////////////////////////// 

void FDungeonFlowEditorUtils::InitializeFlowAsset(UDungeonFlow* InAsset) {
    InAsset->MissionGrammar = NewObject<UGraphGrammar>(InAsset, "MissionGrammar");
    InitializeMissionGrammar(InAsset->MissionGrammar);
}

void FDungeonFlowEditorUtils::InitializeMissionGrammar(UGraphGrammar* InGrammar) {
...
UGraphGrammarProduction* FDungeonFlowEditorUtils::AddNewRule(UGraphGrammar* InGrammar) {
...
UGrammarNodeType* FDungeonFlowEditorUtils::AddNodeType(UGraphGrammar* InGrammar) {
...
void FDungeonFlowEditorUtils::InitializeProductionRule(UGraphGrammarProduction* InProduction) {
...
UGrammarRuleScript* FDungeonFlowEditorUtils::AddProductionRHSGraph(UGraphGrammarProduction* InProduction) {
...
namespace {
    void DebugLogScriptCompileState(UGrammarScriptGraph* ScriptGraph) {
        UE_LOG(LogDungeonFlowEditorUtils, Log, TEXT("Nodes [%d]"), ScriptGraph->Nodes.Num());
    }

    void DebugLogScriptCompileState(UGraphGrammar* InGrammar) {
        UE_LOG(LogDungeonFlowEditorUtils, Log, TEXT("Compiler State"));

        UE_LOG(LogDungeonFlowEditorUtils, Log, TEXT("Execution Script"));
        ...
        for (UGraphGrammarProduction* Production : InGrammar->ProductionRules) {
            UE_LOG(LogDungeonFlowEditorUtils, Log, TEXT("Production Rule Script: %s"),
                   *Production->RuleName.ToString());

            UE_LOG(LogDungeonFlowEditorUtils, Log, TEXT("Source"));
            DebugLogScriptCompileState(Production->SourceGraph->ScriptGraph);

            for (UGrammarRuleScript* DestGraph : Production->DestGraphs) {
                UE_LOG(LogDungeonFlowEditorUtils, Log, TEXT("Dest"));
                DebugLogScriptCompileState(DestGraph->ScriptGraph);
            }

            UE_LOG(LogDungeonFlowEditorUtils, Log, TEXT("=================="));
        }
    }
}

void FDungeonFlowEditorUtils::CompileGrammarToScript(UGraphGrammar* InGrammar) {
    if (!InGrammar) return;
...
void FDungeonFlowEditorUtils::BuildEdGraphFromScriptImpl(UGrammarScriptGraph* ScriptGraph, UEdGraph_Grammar* EdGraph) {
...
void FDungeonFlowEditorUtils::BuildEdGraphFromScript(UGrammarScriptGraph* ScriptGraph, UEdGraph_Grammar* EdGraph) {
    BuildEdGraphFromScriptImpl<UEdGraphNode_GrammarNode>(ScriptGraph, EdGraph);
}

void FDungeonFlowEditorUtils::BuildDebugEdGraphFromScript(UGrammarScriptGraph* ScriptGraph,
...
UEdGraph_Grammar* FDungeonFlowEditorUtils::GetEdGraph(UGrammarRuleScript* RuleScript) {
    return Cast<UEdGraph_Grammar>(RuleScript->EdGraph);
}

UEdGraph_FlowExec* FDungeonFlowEditorUtils::GetEdGraph(UGrammarExecutionScript* ExecScript) {
    return Cast<UEdGraph_FlowExec>(ExecScript->EdGraph);
}

void FDungeonFlowEditorUtils::InitializeGraphScript(UGrammarRuleScript* InGraphScript) {
    InGraphScript->EdGraph = NewObject<UEdGraph_Grammar>(InGraphScript, "EdGraph");
}

void FDungeonFlowEditorUtils::InitializeExecutionScript(UGrammarExecutionScript* InExecutionScript,
