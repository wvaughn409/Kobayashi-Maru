...
void UEdGraph_DungeonProp::RebuildGraph(UDungeonThemeAsset* DataAsset, TArray<FPropTypeData>& OutProps,
        ...
        for (UEdGraphNode_DungeonActorBase* ActorNode : ActorNodes) {
            ...
            if (ActorNode->SpatialConstraint) {
                ...
                Prop.SpatialConstraint = NewObject<UDungeonSpatialConstraint>(
                    DataAsset, Template->GetClass(), NAME_None, RF_NoFlags, Template);
            }
            else {
            ...
            CloneUObjectArray<UDungeonSpawnLogic>(DataAsset, ActorNode->SpawnLogics, Prop.SpawnLogics);
...
void UEdGraph_DungeonProp::RecreateDefaultMarkerNodes(TSubclassOf<UDungeonBuilder> BuilderClass) {
    ...
    if (BuilderClass) {
        UDungeonBuilder* Builder = NewObject<UDungeonBuilder>(static_cast<UObject*>(GetTransientPackage()),
                                                              BuilderClass);
        if (Builder) {
...
TNodeClass* UEdGraph_DungeonProp::CreateNewNode(const FVector2D& Location) {
