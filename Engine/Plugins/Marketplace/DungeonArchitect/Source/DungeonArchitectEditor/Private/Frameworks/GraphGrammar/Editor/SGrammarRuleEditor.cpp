    ...
    [
        ...
        [
            ...
            [
                ...
                [
                    SNew(SImage)
                    .Image(FDungeonArchitectStyle::Get().GetBrush("DA.FlowEd.RuleArrow"))
                ]
            ]
...
void SGrammarRuleEditor::SetRule(TWeakObjectPtr<UGraphGrammarProduction> InRule) {
    ...
    if (Rule.IsValid()) {
        // Set the LHS Graph
        LHSWidget->SetGraph(FDungeonFlowEditorUtils::GetEdGraph(Rule->SourceGraph));

        // Set the RHS Graphs
        RHSWidgets->ClearChildren();
        for (UGrammarRuleScript* RHSGraph : InRule->DestGraphs) {
            AddRHSWidget(FDungeonFlowEditorUtils::GetEdGraph(RHSGraph));
        }
    }
}
...
FReply SGrammarRuleEditor::OnAddRHSRuleClicked() {
    if (Rule.IsValid()) {
        UGrammarRuleScript* NewRHSGraphScript = FDungeonFlowEditorUtils::AddProductionRHSGraph(Rule.Get());
        AddRHSWidget(FDungeonFlowEditorUtils::GetEdGraph(NewRHSGraphScript));
