...
SLATE_BEGIN_ARGS(SDebugGrammarNodeDoorWidget) {
    ...
    void Construct(const FArguments& InArgs) {
        ...
        const FSlateBrush* DoorIcon = bIsIncomingDoor
                                          ? FDungeonArchitectStyle::Get().GetBrush(TEXT("DA.FlowEd.DebugDoorIconIn"))
                                          : FDungeonArchitectStyle::Get().GetBrush(TEXT("DA.FlowEd.DebugDoorIconOut"));
...
FText SGraphNode_DebugGrammarNode::GetText_AssignedMap() const {
    ...
    if (GrammarNode) {
        UEdGraph_DebugGrammar* DebugGraph = Cast<UEdGraph_DebugGrammar>(GrammarNode->GetGraph());
        bool bIsSelectedNode = (DebugGraph->GetCurrentNode() == GrammarNode);
        if (GrammarNode->bModuleAssigned || bIsSelectedNode) {
            FString LevelName = GrammarNode->ModuleInfo.Level.GetAssetName();
...
void SGraphNode_DebugGrammarDoorNode::UpdateGraphNode() {
    ...
    const FSlateBrush* DoorIcon = FDungeonArchitectStyle::Get().GetBrush(TEXT("DA.FlowEd.DebugDoorIcon"));
