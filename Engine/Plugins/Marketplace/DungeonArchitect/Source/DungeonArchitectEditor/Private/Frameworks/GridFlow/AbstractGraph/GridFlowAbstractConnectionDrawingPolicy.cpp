...
#include "Frameworks/GridFlow/AbstractGraph/GridFlowAbstractConnectionDrawingPolicy.h"
    ...
    if (OutputPin && InputPin) {
        ...
        if (SourceNode && DestNode) {
            ...
            if (AbstractEdGraph && AbstractEdGraph->ScriptGraph.IsValid()) {
                ...
                FGridFlowAbstractLink* ScriptLink = ScriptGraph->GetLink(SourceNode->NodeGuid, DestNode->NodeGuid);
                if (ScriptLink) {
                    if (ScriptLink->State.Type == EGridFlowAbstractLinkType::Unconnected) {
                        bConnected = false; // Set as not connected
                    }
                    else if (ScriptLink->State.Type == EGridFlowAbstractLinkType::OneWay) {
    ...
    for (int32 NodeIndex = 0; NodeIndex < ArrangedNodes.Num(); ++NodeIndex) {
        ...
        if (NodeObj->IsA<UGridFlowAbstractEdGraphNode>()) {
            ...
            for (TSharedPtr<SGridFlowItemOverlay> ItemWidget : NodeItemWidgets) {
                const FGridFlowItem& Item = ItemWidget->GetItem();
            ...
            TArray<SGraphNode_GridFlowAbstractNode::FLinkItemWidgetInfo> LinkItemWidgets = HostWidget->
                GetLinkItemWidgets();
            for (const SGraphNode_GridFlowAbstractNode::FLinkItemWidgetInfo& LinkItemInfo : LinkItemWidgets) {
                ...
                if (DestNodeIdxPtr) {
                    ...
                    const FGridFlowItem& Item = LinkItemInfo.ItemWidget->GetItem();
