...
int32 SDATestRunnerHistogram::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry,
    ...
    const FSlateBrush* FillImage = FDungeonArchitectStyle::Get().GetBrush("GridFlowEditor.TestRunner.BarBody");
    ...
    const float NumBars = DataSource->GetNumBars();
    ...
    // draw the grid lines
    uint32 CountX = static_cast<uint32>((AllottedGeometry.Size.X - LabelBuffer * 2.0f) / NumBars);
    ...
    for (int32 Index = 0; Index < NumBars; ++Index) {
        ...
        float MarkerTextPosX = StartX + (Index + 0.5f) * CountX;
    ...
    for (int32 Index = 0; Index < NumBars; ++Index) {
        ...
        float MarkerPosX = StartX + (Index + (1.0f - BarWidth) * 0.5f) * CountX;
        float CurrentCount = DataSource->GetBarValue(Index);
        FLinearColor BarColor = DataSource->GetBarColor(Index);
        float SizeY = static_cast<float>(CurrentCount) / static_cast<float>(TotalCount) * (AllottedGeometry
                                                                                           .GetLocalSize().Y -
            LabelBuffer * 2.0f);
        FSlateDrawElement::MakeBox(
            OutDrawElements,
