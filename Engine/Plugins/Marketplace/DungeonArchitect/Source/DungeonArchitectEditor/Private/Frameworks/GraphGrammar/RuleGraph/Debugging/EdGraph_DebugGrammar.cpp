...
using namespace SnapMapDiagnostics;

UEdGraph_DebugGrammar::UEdGraph_DebugGrammar() {
    Diagnostics = MakeShareable(new FDiagnostics);
}
...
void UEdGraph_DebugGrammar::ExecuteStep() {
    ...
    while (bExecuteNextStep) {
        ...
        if (State.StepIndex + 1 < Diagnostics->Steps.Num()) {
            ...
            const FDiagStep& Step = Diagnostics->Steps[State.StepIndex];
            if (Step.Function == EFunctionType::MoveToNode) {
                TSharedPtr<FPayload_MoveToNode> Payload = StaticCastSharedPtr<FPayload_MoveToNode>(Step.Payload);
                if (Payload.IsValid()) {
            ...
            else if (Step.Function == EFunctionType::BacktrackFromNode) {
                TSharedPtr<FPayload_BacktrackFromNode> Payload = StaticCastSharedPtr<FPayload_BacktrackFromNode>(
                    Step.Payload);
                if (Payload.IsValid()) {
                    ProcessBacktrack(Payload);
                }
            }
            else if (Step.Function == EFunctionType::AssignModule) {
                TSharedPtr<FPayload_AssignModule> Payload = StaticCastSharedPtr<FPayload_AssignModule>(Step.Payload);
                if (Payload.IsValid()) {
                    ProcessAssignModule(Payload);
                }
            }
            else if (Step.Function == EFunctionType::RejectModule) {
                TSharedPtr<FPayload_RejectModule> Payload = StaticCastSharedPtr<FPayload_RejectModule>(Step.Payload);
                if (Payload.IsValid()) {
...
void UEdGraph_DebugGrammar::ProcessMoveToNode(TSharedPtr<FPayload_MoveToNode> InPayload) {
...
void UEdGraph_DebugGrammar::ProcessBacktrack(TSharedPtr<FPayload_BacktrackFromNode> InPayload) {
    State.NodeStack.Pop();
...
void UEdGraph_DebugGrammar::ProcessAssignModule(TSharedPtr<FPayload_AssignModule> InPayload) {
    if (State.CurrentNode.IsValid()) {
        SetCurrentNodeStatusMessage("Assign Module");
        State.CurrentNode->bModuleAssigned = true;
        State.CurrentNode->ModuleInfo = InPayload->ModuleInfo;
        ...
        OnVisualizeLoadLevel.ExecuteIfBound(State.CurrentNode->NodeId, InPayload->ModuleInfo,
...
void UEdGraph_DebugGrammar::ProcessRejectModule(TSharedPtr<FPayload_RejectModule> InPayload) {
    if (State.CurrentNode.IsValid()) {
        FString Reason;
        switch (InPayload->Reason) {
        case EModuleRejectReason::BoundsCollide:
            Reason = "Bounds Collide";
            break;

        case EModuleRejectReason::NotEnoughDoorsAvailable:
            Reason = "Not enough doors";
            break;

        case EModuleRejectReason::NoModuleAvailable:
            Reason = "Not modules found";
            break;

        case EModuleRejectReason::CannotBuildSubTree:
...
void UEdGraph_DebugGrammar::HilightCurrentNode(const FColor& InColor) {
    ...
    if (State.CurrentNode.IsValid() && State.CurrentNode->bModuleAssigned) {
        Bounds = State.CurrentNode->ModuleInfo.ModuleBounds.TransformBy(State.CurrentNode->WorldTransform);
    }
