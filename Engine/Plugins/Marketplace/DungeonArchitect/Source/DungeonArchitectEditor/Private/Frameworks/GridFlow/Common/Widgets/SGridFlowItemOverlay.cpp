...
void SGridFlowItemOverlay::Construct(const FArguments& InArgs, const FGridFlowItem& InItem) {
    Item = InItem;
    Selected = InArgs._Selected;

    // TODO: Move this to DA style sheet
    const FSlateBrush* BodyBrush = FDungeonArchitectStyle::Get().GetBrush(TEXT("GridFlowEditor.ItemWidget.Body"));
    const FSlateBrush* SelectionBrush = FDungeonArchitectStyle::Get().GetBrush(
        TEXT("GridFlowEditor.ItemWidget.Body.Selected"));
    FSlateFontInfo Font = FDungeonArchitectStyle::Get().GetFontStyle("GridFlowEditor.ItemWidget.Text");
    if (Item.CustomInfo.FontScale != 1.0f) {
...
FReply SGridFlowItemOverlay::OnMouseButtonDoubleClick(const FGeometry& InMyGeometry,
...
FSlateColor SGridFlowItemOverlay::GetColor() const {
    return FGridFlowItemVisuals::GetBackgroundColor(Item, bHovered);
}

FSlateColor SGridFlowItemOverlay::GetTextColor() const {
    return FGridFlowItemVisuals::GetTextColor(Item, bHovered);
}

FText SGridFlowItemOverlay::GetItemText() const {
    return FText::FromString(FGridFlowItemVisuals::GetText(Item));
}
