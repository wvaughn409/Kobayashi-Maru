...
struct TDANumericUnitTypeInterface : TDefaultNumericTypeInterface<NumericType> {
    ...
    FString ToString(const NumericType& Value) const override {
    ...
    TOptional<NumericType> FromString(const FString& InString, const NumericType& InExistingValue) override {
        if (UnderlyingUnits == EUnit::Unspecified) {
...
public:
    ...
    FName GetName() const override { return NAME_None; }
    bool InitiallyCollapsed() const override { return false; }
    void Tick(float DeltaTime) override;

    void SetOnRebuildChildren(FSimpleDelegate OnRebuildChildren) override {
    }
