...
struct DUNGEONARCHITECTRUNTIME_API FDungeonSchemaAction_NewNode : public FEdGraphSchemaAction {
    ...
    UEdGraphNode* PerformAction(class UEdGraph* ParentGraph, TArray<UEdGraphPin*>& FromPins, FVector2D Location,
                                bool bSelectNewNode = true) override;
...
public:
    ...
    static void AddAction(FString Title, FString Tooltip, TArray<TSharedPtr<FEdGraphSchemaAction>>& OutActions,
        ...
        TSharedPtr<FDungeonSchemaAction_NewNode> NewActorNodeAction = AddNewNodeAction(
            OutActions, Category, MenuDesc, TooltipText);
