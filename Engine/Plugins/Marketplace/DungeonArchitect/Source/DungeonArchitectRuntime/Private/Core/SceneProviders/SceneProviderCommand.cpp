...
FName SceneProviderCommand::TagComplexActor = "Dungeon-Complex-Actor";

void SceneProviderCommand::Execute(UWorld* World) {
    ExecuteImpl(World);
}

void SceneProviderCommand::PostInitializeActor(AActor* Actor) {
...
void SceneProviderCommand::ExecuteSpawnLogics(AActor* SpawnedActor, ADungeon* InDungeon,
...
void SceneProviderCommand::AddReferencedObjects(FReferenceCollector& Collector) {
    Collector.AddReferencedObject(Dungeon);
    Collector.AddReferencedObjects(Context.SpawnLogics);
}

void SceneProviderCommand::AddNodeTag(AActor* Actor, const FName& NodeId, bool bApplyPrefix /* = true */) {
    if (Actor) {
...
void SceneProviderCommand::MoveToFolder(ADungeon* Dungeon, AActor* ActorToMove) {
#if WITH_EDITOR
...
void SceneProviderCommand::TagAsComplexObject(AActor* Actor) {
...
void SceneProviderCommand_CreateMesh::AddReferencedObjects(FReferenceCollector& Collector) {
    SceneProviderCommand::AddReferencedObjects(Collector);
...
void SceneProviderCommand_AddParticleSystem::ExecuteImpl(UWorld* World) {
    ULevel* Level = Dungeon ? Dungeon->GetLevel() : nullptr;
    AEmitter* ParticleEmitter = AddActor<AEmitter>(World, Level, Context.transform, Context.NodeId);
...
void SceneProviderCommand_ReuseActorTemplate::ExecuteImpl(UWorld* World) {
    if (ClassTemplate && ActorToReuse && !ActorToReuse->IsA(ClassTemplate)) {
        ...
        SceneProviderCommand_AddActor AddActorCommand(Dungeon, Context, ClassTemplate);
...
void SceneProviderCommand_ReuseClonedActor::ExecuteImpl(UWorld* World) {
    if (ActorTemplate && !ActorToReuse->IsA(ActorTemplate->GetClass())) {
        ...
        SceneProviderCommand_CloneActor CloneActorCommand(Dungeon, Context, ActorTemplate);
