...
/*
void ADungeon::RebuildNavigation()
{
...
void ADungeon::ProcessLevelStreaming() {
    if (LevelStreamingModel) {
        UWorld* World = GetWorld();
        if (World->IsGameWorld()) {
            TArray<FVector> PlayerLocations;
            GetPlayerLocations(World, PlayerLocations);

            FDungeonLevelStreamer Streamer(LevelStreamingConfig);
            Streamer.Process(LevelStreamingModel, PlayerLocations);
        }
    }
}
