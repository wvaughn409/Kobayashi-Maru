...
void UGridFlowTilemap::Initialize(int32 InWidth, int32 InHeight) {
    Width = InWidth;
    Height = InHeight;
    Cells.AddDefaulted(Width * Height);
...

    {
        ...
        EdgesHorizontal.AddDefaulted(EdgesWidth * EdgesHeight);
        EdgesVertical.AddDefaulted(EdgesWidth * EdgesHeight);
...
void FGridFlowTilemapDistanceField::FindDistanceFromEdge(UGridFlowTilemap* Tilemap) {
    ...
    for (int y = 0; y < Tilemap->GetHeight(); y++) {
        for (int x = 0; x < Tilemap->GetWidth(); x++) {
            ...
            if (Cell.CellType == EGridFlowTilemapCellType::Floor) {
                ...
                for (int i = 0; i < 4; i++) {
                    int32 cx = x + ChildOffsets[i * 2 + 0];
                    int32 cy = y + ChildOffsets[i * 2 + 1];
    ...
    while (Queue.Dequeue(CellCoord)) {
        ...
        for (int i = 0; i < 4; i++) {
            int32 nx = x + ChildOffsets[i * 2 + 0];
            int32 ny = y + ChildOffsets[i * 2 + 1];
