...
void UFloorPlanBuilder::BuildLayout() {
    ...
    for (int z = 0; z < NumFloors; z++) {
        ...
        TArray<FloorChunkPtr> FloorChunks;
        {
            ...
            InitialFloor->Bounds.Size = FMathUtils::ToIntVector(floorPlanConfig->BuildingSize);
        ...
        for (FloorChunkPtr Chunk : FloorChunks) {
            TArray<FloorChunkPtr> Stack;
            Stack.Push(Chunk);
            const int32 MinArea = floorPlanConfig->MinRoomSize * floorPlanConfig->MinRoomSize;
            const int32 MaxArea = floorPlanConfig->MaxRoomSize * floorPlanConfig->MaxRoomSize;
