...
class COMPLETERPG_API UManiacManfredExpressoScripts : public UArticyExpressoScripts
{
	GENERATED_BODY()
...
public:
	 ...
	 UManiacManfredExpressoScripts() 
	{
		...
		Conditions.Add(919865247, [&]
		{
			return ConditionOrTrue(
				//Clicking on Niallghas

			);
		});
		Conditions.Add(-50187633, [&]
		{
			return ConditionOrTrue(
				//Clicking on Lach
		...
		Conditions.Add(-1577312080, [&]
		{
			return ConditionOrTrue(
				(*GameState->awake) == false && (*GameState->friendOfNiallghas) == false && (*GameState->friendOfLachlainn) == false && (*GameState->friendOfEarc) == false && (*GameState->friendOfBrianan) == false && (*GameState->friendOfAngaidh) == false
			);
		});
		...
		Conditions.Add(-196408691, [&]
		{
			return ConditionOrTrue(
				(*GameState->awake) == true && (*GameState->friendOfNiallghas) == false && (*GameState->friendOfLachlainn) == false && (*GameState->friendOfEarc) == false && (*GameState->friendOfBrianan) == false && (*GameState->friendOfAngaidh) == false
			);
		});
 ...
 (*GameState->therapist_convinced) = true;
		...
		Conditions.Add(-1837498558, [&]
		{
			return ConditionOrTrue(
				(*GameState->friendOfNiallghas) == true && (*GameState->friendOfLachlainn) == true && (*GameState->friendOfEarc) == false
			);
		});
		Instructions.Add(-1754807136, [&]
		{
			(*GameState->friendOfEarc) = true;
		});
		Conditions.Add(392284443, [&]
		{
			return ConditionOrTrue(
				(*GameState->friendOfNiallghas) == false && (*GameState->friendOfLachlainn) == false && (*GameState->friendOfEarc) == false
			);
		});
		Conditions.Add(-200280055, [&]
		{
			return ConditionOrTrue(
				(*GameState->friendOfNiallghas) == true && (*GameState->friendOfLachlainn) == false && (*GameState->friendOfEarc) == false || (*GameState->friendOfNiallghas) == false && (*GameState->friendOfLachlainn) == true && (*GameState->friendOfEarc) == false
			);
		});
		Conditions.Add(906803686, [&]
		{
			return ConditionOrTrue(
				(*GameState->friendOfNiallghas) == true && (*GameState->friendOfLachlainn) == true && (*GameState->friendOfEarc) == true && (*GameState->friendOfBrianan) == false
			);
		});
		Instructions.Add(-787932898, [&]
		{
			(*GameState->friendOfBrianan) = true;
		});
		Conditions.Add(1134756002, [&]
		{
			return ConditionOrTrue(
				(*GameState->friendOfNiallghas) == false && (*GameState->friendOfBrianan) == false || (*GameState->friendOfLachlainn) == false && (*GameState->friendOfBrianan) == false || (*GameState->friendOfEarc) == false && (*GameState->friendOfBrianan) == false
			);
		});
		Conditions.Add(-665018328, [&]
		{
			return ConditionOrTrue(
				(*GameState->friendOfBrianan) == true
			);
		});
		Conditions.Add(-738872614, [&]
		{
			return ConditionOrTrue(
				(*GameState->friendOfNiallghas) == false
			);
		});
		Instructions.Add(-1952174890, [&]
		{
			(*GameState->awake) = true;
 (*GameState->friendOfNiallghas) = true;
		});
		Conditions.Add(-685738746, [&]
		{
			return ConditionOrTrue(
				(*GameState->friendOfNiallghas) == true
			);
		});
		Conditions.Add(1923444577, [&]
		{
			return ConditionOrTrue(
				(*GameState->friendOfNiallghas) == true && (*GameState->friendOfLachlainn) == false
			);
		});
		Instructions.Add(-2051970257, [&]
		{
			(*GameState->friendOfLachlainn) = true;
		});
		Conditions.Add(541777836, [&]
		{
			return ConditionOrTrue(
				(*GameState->friendOfNiallghas) == false && (*GameState->friendOfLachlainn) == false
			);
		});
		Conditions.Add(-916573344, [&]
		{
			return ConditionOrTrue(
				(*GameState->friendOfNiallghas) == true && (*GameState->friendOfLachlainn) == true
			);
		});
		Conditions.Add(-502369394, [&]
		{
			return ConditionOrTrue(
				(*GameState->friendOfNiallghas) == true && (*GameState->friendOfLachlainn) == true && (*GameState->friendOfEarc) == true && (*GameState->friendOfBrianan) == true && (*GameState->friendOfAngaidh) == false
			);
		});
		Instructions.Add(1680023067, [&]
		{
			(*GameState->friendOfAngaidh) = true;
		});
		Conditions.Add(-1572276632, [&]
		{
			return ConditionOrTrue(
				(*GameState->friendOfNiallghas) == false  && (*GameState->friendOfAngaidh) == false || (*GameState->friendOfLachlainn) == false  && (*GameState->friendOfAngaidh) == false || (*GameState->friendOfEarc) == false  && (*GameState->friendOfAngaidh) == false || (*GameState->friendOfBrianan) == false && (*GameState->friendOfAngaidh) == false
			);
		});
		Conditions.Add(1405321368, [&]
		{
			return ConditionOrTrue(
				(*GameState->friendOfNiallghas) == true && (*GameState->friendOfLachlainn) == true && (*GameState->friendOfEarc) == true && (*GameState->friendOfBrianan) == true && (*GameState->friendOfAngaidh) == true
			);
		});
 ...
 (*GameState->therapist_knockedOut2) = true;
		...
		Conditions.Add(-330592033, [&]
		{
			return ConditionOrTrue(
				(*GameState->friendOfEarc) == false
			);
		});
		Conditions.Add(-1640858218, [&]
		{
			return ConditionOrTrue(
				(*GameState->friendOfEarc) == true
			);
 ...
 setProp(getObj("Chr_Manfred"), "Morale.MoraleValue", getProp(getObj("Chr_Manfred"), "Morale.MoraleValue") - 10);
		...
		Conditions.Add(1302440859, [&]
		{
			return ConditionOrTrue(
				(*GameState->defaultFalse) == false
			);
		});
		Conditions.Add(2054318494, [&]
		{
			return ConditionOrTrue(
				(*GameState->defaultFalse) == false && (*GameState->friendOfEarc) == false
			);
		});
